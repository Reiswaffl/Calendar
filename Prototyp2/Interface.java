...
public class Interface {
    ...
    public DayObject GetDayInformation(String date) { // yy,mm,dd
        DayObject dO = new DayObject();
                        ...
                        for(int p = 0; p < periods.getLength(); p++){
                            if(eday.getElementsByTagName("period").item(p) != null) {
                               // System.out.println(eday.getElementsByTagName("period").item(p).getTextContent());
                                 dO.Add(eday.getElementsByTagName("period").item(p).getTextContent());
    ...
    public PeriodObject GetPeriodInformation(String date, String period){
        PeriodObject pO = null;
        int idate = Integer.parseInt(date);
        int year = idate - idate%10000;
        year = year / 10000;
        year += 2000;
                        ...
                        for(int p = 0; p < periods.getLength(); p++){
                            ...
                            if(nperiod.getNodeType() == Node.ELEMENT_NODE){
                                ...
                                if(eperiod.getAttribute("time").equals(period)){
                                    ...
                                    pO = new PeriodObject(eperiod.getTextContent());
                                }
                            }
                        }
                    }
    ...
    public String getNextFreeDay(){
        ...
        if(m.getChildNodes() != null)
        {
            ...
            Element em = (Element) m;
            Element ey = (Element) y;
            return(lastday+1)+"."+ em.getAttribute("name") +"."+ ey.getAttribute("id");
        }
        return "Leider nicht in dem aktuellen Monat vorhanden c ya"; // Noch krass improvisiert (noch verbessern)
